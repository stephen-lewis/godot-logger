# Taken and modified from godot-logger plugin:
...
class ExternalSink:
	enum QueueModes { NONE = 0, ALL = 1, SMART = 2 }

	var name := ""
	var queue_mode := QueueModes.NONE
	var buffer := PackedStringArray()
	var buffer_idx := 0

	func _init(_name : String, _queue_mode := QueueModes.NONE) -> void:
		name = _name
		queue_mode = _queue_mode

	func flush_buffer() -> void:
		## Flush the buffer, i.e.: write its contents to the target external sinl.
		print("[ERROR] [logger] Using method which has to be overriden in your custom sink")

	func write(_output : String, _level : LogLevels) -> void:
		## Write the string at the end of the sink (append mode), following the queue mode.
		print("[ERROR] [logger] Using method which has to be overriden in your custom sink")

	func set_queue_mode(new_mode : QueueModes) -> void:
		queue_mode = new_mode

	func get_queue_mode() -> QueueModes:
		return queue_mode
	...
	func get_config() -> Dictionary:
		return { "queue_mode": get_queue_mode(), }
...
class Logfile:
	...
	func _init(_path : String, _queue_mode := QueueModes.NONE) -> void:
	...
	func validate_path(path : String) -> bool:
		## Validate the path given as argument, making it possible to write to the designated file or folder. Returns whether the path is valid.
	...
	func flush_buffer() -> void:
		...
		var temp_file := _open_file(log_path)
		if temp_file:
			temp_file.seek_end()
	...
	func write(output : String, level : LogLevels) -> void:
		## Write the string at the end of the file (append mode), following the queue mode.
		var queue_action := queue_mode
		if queue_action == QueueModes.SMART:
			if level >= LogLevels.WARN:  # Don't queue warnings and errors
				queue_action = QueueModes.NONE
				flush_buffer()
			else:  # Queue the log, not important enough for "smart"
				queue_action = QueueModes.ALL

		if queue_action == QueueModes.NONE:
			var temp_file := _open_file(log_path)
			if temp_file == null:
				return
			else:
		...
		if queue_action == QueueModes.ALL:
...
class Module:
	...
	var name := ""
	var output_level := LogLevels.VERBOSE
	var output_strategies := []
	var external_sink : ExternalSink = null

	func _init(_name : String, _output_level : LogLevels, _output_strategies, _external_sink : ExternalSink) -> void:
		name = _name
	...
	func set_output_level(level : LogLevels) -> void:
		## Set the custom minimal level for the output of the module. All levels greater or equal to 
		## the given once will be output based on their respective strategies, while levels lower 
		## than the given one will be discarded.
		if not level in range(0, LEVELS.size()):
			print("[ERROR] [%s] The level must be comprised between 0 and %d." % [PLUGIN_NAME, LEVELS.size() - 1])
			return
		output_level = level

	func get_output_level() -> LogLevels:
		return output_level
	...
	func set_output_strategy(output_strategy_mask : int, level : LogLevels = -1) -> void:
		## Set the output strategy for the given level or (by default) all levels of the module.
		if not output_strategy_mask in range(0, MAX_STRATEGY + 1):
			print("[ERROR] [%s] The output strategy mask must be comprised between 0 and %d." % [PLUGIN_NAME, MAX_STRATEGY])
			return
	...
	func get_output_strategy(level : LogLevels = -1):
		if level == -1:
...
# Configuration
var default_output_level := LogLevels.INFO

var default_output_strategies := [STRATEGY_PRINT, STRATEGY_PRINT, STRATEGY_PRINT, STRATEGY_PRINT, STRATEGY_PRINT]
var default_logfile_path : String = "user://%s.log" % ProjectSettings.get_setting("application/config/name")
...
func put(level : LogLevels, message : String, module := default_module_name, error_code := -1):
...
func add_module(module_name : String, output_level := default_output_level, output_strategies = default_output_strategies, logfile : ExternalSink = null) -> Module:
	...
	if modules.has(module_name):
		info("The module '%s' already exists; discarding the call to add it anew." % module_name, PLUGIN_NAME)
	else:
		if logfile == null:
			logfile = get_external_sink(default_logfile_path)
		modules[module_name] = Module.new(module_name, output_level, output_strategies, logfile)
...
func add_logfile(logfile_path := default_logfile_path) -> ExternalSink:
	## Add a new logfile that can then be attached to one or more modules. Returns a reference to the instanced logfile.
...
func get_external_sink(_external_sink_name) -> ExternalSink:
...
func set_default_output_strategy(output_strategy_mask : int, level : LogLevels = -1) -> void:
	## Set the default output strategy mask of the given level or (by default) all levels for all modules without a custom strategy.
	if not output_strategy_mask in range(0, MAX_STRATEGY + 1):
...
func get_default_output_strategy(level : LogLevels):
	## Get the default output strategy mask of the given level or (by default) all levels for all modules without a custom strategy.
	return default_output_strategies[level]

func set_default_output_level(level : LogLevels) -> void:
	## Set the default minimal level for the output of all modules without a custom output level.
	## All levels greater or equal to the given once will be output based on their respective 
	## strategies, while levels lower than the given one will be discarded.
	if not level in range(0, LEVELS.size()):
		error("The level must be comprised between 0 and %d." % (LEVELS.size() - 1), PLUGIN_NAME)
		return
...
func get_default_output_level() -> LogLevels:
	## Get the default minimal level for the output of all modules without a custom output level.
...
func format(template : String, level : LogLevels, module : String, message : String, error_code := -1) -> String:
	var output := template
...
func save_config(configfile := default_configfile_path) -> int:
	## Save the default configuration as well as the set of modules and their respective configurations.
...
func load_config(configfile := default_configfile_path) -> int:
	## Load the configuration as well as the set of defined modules and their respective 
	## configurations. The expect file contents must be those produced by the ConfigFile API. 
	## Returns an error code (OK or some ERR_*).
	# Look for the file
